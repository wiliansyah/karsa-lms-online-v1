...
const Dashboard = ({ user, setView, onToggleAccess }) => {
  ...
  const UserApp = () => {
    ...
    const [tourState, setTourState] = useState({
        run: false,
        steps: [],
        stepIndex: 0 // Initialize stepIndex
    });
    ...
    const handleJoyrideCallback = (data) => {
        const { status, index } = data; // Destructure index to update stepIndex
        ...
        // Update current step index in state to keep track
        if (data.action === ACTIONS.NEXT || data.action === ACTIONS.PREV) {
            setTourState(prev => ({ ...prev, stepIndex: index + (data.action === ACTIONS.NEXT ? 1 : -1) }));
        }
    };
    ...
    // 5. NEW FUNCTION: MANUAL GLOBAL TOUR TRIGGER
    ...
    return (
      <div className="flex min-h-screen bg-slate-50 text-slate-800 font-sans overflow-hidden">
        ...
        <Joyride
          ...
          run={tourState.run}
          stepIndex={tourState.stepIndex} // Control step index
          continuous={true}
          showSkipButton={true}
          showProgress={true}
        ...
        <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
          <header className="h-16 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between px-6 shrink-0 z-30">
            ...
            <div className="flex items-center gap-4">
                {/* --- MODIFIKASI: TOMBOL BANTUAN/HELP --- */}
                <button 
                    onClick={handleManualTourStart}
                    className="flex items-center gap-1.5 px-3 py-1 bg-white text-blue-600 rounded-full text-xs font-bold border border-blue-100 shadow-sm hover:bg-blue-50 transition-colors"
                >
                    <HelpCircle size={16} /> <span className="hidden md:inline">Tutorial</span>
                </button>
                {/* --------------------------------------- */}
