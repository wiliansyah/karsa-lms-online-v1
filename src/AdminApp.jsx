...
// --- 4. TUTORIAL SCENARIOS & DATA (UX CONTENT STRATEGY) ---
const TOUR_SCENARIOS = {
    dashboard: [
        { target: 'body', content: <div className="text-left"><h4 className="font-bold mb-2">👋 Welcome, Admin!</h4><p className="text-sm">This is your HR Command Center. We prioritize <b>Compliance</b> and <b>Growth</b>. Let's look at the health of your organization.</p></div>, placement: 'center' },
        { target: '.tour-stats-overview', content: <div className="text-left"><h4 className="font-bold text-sm mb-1">Vital Signs</h4><p className="text-xs">Quickly identify <b>'At Risk'</b> employees (low compliance) and <b>Pending Requests</b> that block budget utilization.</p></div>, placement: 'bottom' },
        { target: '.tour-dashboard-analytics-btn', content: <div className="text-left"><h4 className="font-bold text-sm mb-1">Deep Dive</h4><p className="text-xs">Drill down into department-specific skill gaps. Use this to justify next quarter's training budget.</p></div>, placement: 'top' },
    ],
    employees: [
        { target: '.tour-emp-search', content: 'Find talent by name or filter by department context.', placement: 'bottom' },
        { target: '.tour-emp-add', content: 'Onboard new staff. Note: Correct email input triggers the SSO provision system.', placement: 'left' },
        { target: '.tour-emp-table', content: 'Real-time compliance tracking. Click any row to manage individual assignments.', placement: 'top' },
    ],
    curriculum: [
        { target: '.tour-curr-create', content: 'Design new learning paths. You can upload SCORM or Video content.', placement: 'left' },
        { target: '.tour-curr-assign', content: '<b>Bulk Action:</b> Push mandatory compliance training to an entire department in one click.', placement: 'bottom' },
        { target: '.tour-curr-grid', content: 'Active vs Draft courses. Only Active courses are visible on the Employee App.', placement: 'top' },
    ],
    requests: [
        { target: '.tour-req-table', content: 'External training requests submitted by employees via the mobile app.', placement: 'top' },
        { target: '.tour-req-detail', content: '<b>Compliance Check:</b> Ensure "Leader Approval" is attached before releasing budget.', placement: 'left' },
    ],
    // --- MODAL SPECIFIC SCENARIOS ---
    'add-employee': [
        { target: '.modal-tour-name', content: '<b>Official Name:</b> Must match legal ID for certificate generation.', placement: 'right' },
        { target: '.modal-tour-dept', content: '<b>Department Logic:</b> Selecting this auto-assigns the "Onboarding Curriculum" specific to that role (e.g., Kitchen Staff gets HACCP immediately).', placement: 'top' },
        { target: '.modal-tour-role', content: 'Defines access level permissions in the system.', placement: 'top' },
    ],
    'create-course': [
        { target: '.modal-tour-category', content: '<b>Category Logic:</b> "Mandatory" courses affect the Compliance Score. "Soft Skills" do not penalize the user if skipped.', placement: 'right' },
        { target: '.modal-tour-audience', content: '<b>Targeting:</b> "All Staff" sends a push notification to everyone. Use carefully to avoid notification fatigue.', placement: 'left' },
        { target: '.modal-tour-upload', content: 'Supports MP4, PDF, and HTML5. Ensure files are under 500MB for mobile optimization.', placement: 'top' }
    ]
};


// --- 5. MODALS & FORMS (Enhanced with Tour Triggers) ---

const AddEmployeeModal = ({ isOpen, onClose, onSave, triggerTour }) => {
            ...
            <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl animate-pop">
                <div className="p-5 border-b border-slate-100 flex justify-between items-center">
                    <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => triggerTour('add-employee')} className="text-xs font-bold text-[#D12027] bg-red-50 border border-red-100 px-2 py-1 rounded-md flex items-center gap-1 hover:bg-red-100 transition-colors"><HelpCircle size={12}/> Guide Me</button>
                    </div>
                    <button onClick={onClose}><X size={20} className="text-slate-400 hover:text-red-500"/></button>
                </div>
                <div className="p-6 space-y-4">
                    <div className="modal-tour-name"><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Full Name</label><input value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full p-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-red-100 outline-none"/></div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="modal-tour-dept"><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Department</label><select value={formData.dept} onChange={e => setFormData({...formData, dept: e.target.value})} className="w-full p-2 border border-slate-200 rounded-lg outline-none"><option>Frontliner</option><option>Kitchen</option><option>Operational</option><option>Warehouse</option></select></div>
                        <div className="modal-tour-role"><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Role</label><input value={formData.role} onChange={e => setFormData({...formData, role: e.target.value})} className="w-full p-2 border border-slate-200 rounded-lg outline-none"/></div>
                    </div>
                </div>
...
const CreateCourseModal = ({ isOpen, onClose, onSave, triggerTour }) => {
            ...
            <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl animate-pop overflow-hidden">
                <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                    <div className="flex items-center gap-3">
                        ...
                        {/* Contextual Help Button */}
                        <button onClick={() => triggerTour('create-course')} className="text-xs font-bold text-[#D12027] bg-white border border-[#D12027] px-3 py-1 rounded-full flex items-center gap-1 hover:bg-red-50 transition-colors shadow-sm"><HelpCircle size={12}/> Explain Form Logic</button>
                    </div>
                    <button onClick={onClose}><X size={20} className="text-slate-400 hover:text-red-500"/></button>
                </div>
                <div className="p-6 md:p-8">
                    {step === 1 ? (
                        <div className="space-y-4 animate-slideInRight">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="modal-tour-category"><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Category</label><select value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})} className="w-full p-3 border border-slate-200 rounded-xl outline-none bg-white"><option>Mandatory</option><option>Soft Skill</option><option>Technical</option><option>Managerial</option></select></div>
                                <div className="modal-tour-audience"><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Target Audience</label><select value={formData.level} onChange={e => setFormData({...formData, level: e.target.value})} className="w-full p-3 border border-slate-200 rounded-xl outline-none bg-white"><option>All Staff</option><option>Frontliner</option><option>Manager</option><option>Kitchen Staff</option></select></div>
                            </div>
                        ...
                        <div className="space-y-4 animate-slideInRight">
                            ...
                            <div className="modal-tour-upload bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl p-8 flex flex-col items-center justify-center text-center"><div className="bg-white p-3 rounded-full shadow-sm mb-3"><Download size={24} className="text-slate-400"/></div><p className="font-bold text-slate-600">Upload Course Material</p><p className="text-xs text-slate-400">Drag & drop files or <span className="text-[#D12027] cursor-pointer hover:underline">Browse</span></p></div>
                        </div>
                    )}
                </div>
...
// --- 6. SUB-VIEWS & MANAGERS ---

const EmployeeManager = ({ employees, courses, onAddEmployee, onTourTrigger }) => {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full lg:h-[calc(100vh-140px)] gap-6 animate-fadeIn pb-20 lg:pb-0">
      ...
      {/* Passing triggerTour so Modal can call specific Help Steps */}
      <AddEmployeeModal isOpen={addModalOpen} onClose={() => setAddModalOpen(false)} onSave={onAddEmployee} triggerTour={onTourTrigger} />
    </div>
  );
};
...
const CurriculumManager = ({ courses, employees, onAddCourse, onUpdateCourseStatus, onTourTrigger }) => {
  ...
  return (
    <div className="space-y-6 animate-fadeIn relative pb-20">
      ...
      {/* Trigger Tour passed */}
      <CreateCourseModal isOpen={showCreateModal} onClose={() => setShowCreateModal(false)} onSave={onAddCourse} triggerTour={onTourTrigger} />
...
const AdminApp = () => {
  ...
  // --- TUTORIAL ENGINE (Enhanced) ---
  const [runTour, setRunTour] = useState(false);
  const [tourSteps, setTourSteps] = useState([]);
  const [tourKey, setTourKey] = useState(0); // Used to force restart tours

  // 1. Detect View Changes & Load specific tour
  useEffect(() => {
      const seenKey = `hasSeen_${activeView}`;
      const hasSeen = localStorage.getItem(seenKey);
      
      if (!hasSeen && TOUR_SCENARIOS[activeView]) {
          setTourSteps(TOUR_SCENARIOS[activeView]);
          setRunTour(true);
      }
  }, [activeView]);

  // 2. Global Tutorial Trigger (Called by Modals or Help Button)
  const triggerTutorial = (scenarioKey) => {
      if (TOUR_SCENARIOS[scenarioKey]) {
          setTourSteps(TOUR_SCENARIOS[scenarioKey]);
          setRunTour(true);
          setTourKey(prev => prev + 1); // Force restart if it's the same modal
      }
  };
  ...
  const handleJoyrideCallback = (data) => {
      ...
      if ([STATUS.FINISHED, STATUS.SKIPPED].includes(status)) {
          setRunTour(false);
          // Only save "seen" status for main page views, not modals (so users can ask for help again)
          if(Object.keys(TOUR_SCENARIOS).slice(0, 5).includes(activeView)) {
    ...
    <div className="flex h-screen bg-[#f8fafc] overflow-hidden text-slate-800 font-sans">
      ...
      <Joyride
        ...
        styles={{
          options: {
            primaryColor: '#D12027',
            // High Z-Index to stay above Modals (z-70)
            zIndex: 110, 
            arrowColor: '#fff',
            backgroundColor: '#fff',
          ...
          buttonBack: { color: '#64748b', marginRight: 10 }
        }}
      />
      ...
      <div className="flex-1 flex flex-col h-screen relative w-full">
        <header className="h-16 lg:h-20 bg-white/80 backdrop-blur border-b border-slate-200 flex items-center justify-between px-4 lg:px-8 z-20 sticky top-0">
          ...
          <div className="tour-profile flex items-center gap-4 lg:gap-6">
            <button onClick={() => triggerTutorial(activeView)} className="text-sm font-bold text-[#D12027] bg-red-50 px-3 py-1.5 rounded-lg border border-red-100 hover:bg-red-100 transition-colors">Help ?</button>
        ...
        <main className="flex-1 overflow-y-auto p-4 lg:p-8 scroll-smooth w-full">
           <div className="max-w-7xl mx-auto">
             ...
             {/* Pass onTourTrigger to children so they can trigger modal tours */}
             {activeView === 'employees' && <EmployeeManager employees={employees} courses={courses} onAddEmployee={handleAddEmployee} onTourTrigger={triggerTutorial} />}
             {activeView === 'curriculum' && <CurriculumManager courses={courses} employees={employees} onAddCourse={handleAddCourse} onUpdateCourseStatus={handleUpdateCourseStatus} onTourTrigger={triggerTutorial} />}
