...
const GlobalStyles = () => (
  <style>{`
    ...
    /* Custom Tour Styles */
    .react-joyride__tooltip { border-radius: 12px !important; box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04) !important; border: 1px solid #e2e8f0; }
    .react-joyride__tooltip h4 { font-weight: 800 !important; color: #1e293b !important; font-size: 18px !important; margin-bottom: 8px !important; }
    .react-joyride__tooltip div { font-size: 14px !important; color: #475569 !important; line-height: 1.5 !important; }
    .react-joyride__button--primary { background-color: ${KARSA_RED} !important; border-radius: 8px !important; font-weight: bold !important; }
    .react-joyride__button--skip { color: #94a3b8 !important; font-weight: bold !important; font-size: 12px !important; }
  `}</style>
);
...
// --- 4. MODALS & FORMS WITH SUB-FEATURE TUTORIALS ---
...
// Modal: Create Course (Contains its own Tutorial)
const CreateCourseModal = ({ isOpen, onClose, onSave }) => {
    ...
    const modalSteps = [
        { target: '.tour-modal-step1', content: 'Step 1: Tentukan Identitas Kursus. Pilih Kategori yang tepat (Mandatory/Soft Skill) untuk pelaporan yang akurat.', placement: 'right', disableBeacon: true },
        { target: '.tour-modal-step2', content: 'Step 2: Upload materi (Video/PDF). Pastikan format file didukung dan ukuran di bawah 50MB.', placement: 'left' }
    ];

    useEffect(() => {
        if(isOpen) { 
            // Check if user has seen this modal tutorial
    ...
    return (
        <div className="fixed inset-0 z-[70] bg-black/50 flex items-center justify-center p-4 animate-fadeIn">
            {/* Embedded Joyride for Sub-Feature Tutorial */}
            <Joyride 
                steps={modalSteps} 
                run={runModalTour} 
                continuous 
                ...
                styles={{ options: { zIndex: 10000, primaryColor: KARSA_RED } }}
            />

            <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl animate-pop overflow-hidden relative">
                ...
                <div className="p-6 md:p-8">
                    {step === 1 ? (
                        <div className="tour-modal-step1 space-y-4 animate-slideInRight">
                        ...
                        <div className="tour-modal-step2 space-y-4 animate-slideInRight">
                            <div className="grid grid-cols-2 gap-4">
                ...
                {/* Help Trigger */}
                <button onClick={() => setRunModalTour(true)} className="absolute bottom-5 left-1/2 -translate-x-1/2 text-xs text-slate-400 hover:text-slate-600 flex items-center gap-1"><HelpCircle size={12}/> How to fill this?</button>
            </div>
        </div>
    );
};
...
// Modal: Dept Analytics (Contains its own Tutorial)
const DeptAnalyticsModal = ({ isOpen, onClose }) => {
    const [runAnalyticsTour, setRunAnalyticsTour] = useState(false);
    
    // Sub-Feature Tour Steps for Analytics
    const analyticsSteps = [
        { target: '.tour-deep-radar', content: 'Skill Gap Radar: Membandingkan skill Frontliner vs Kitchen. Titik rendah menunjukkan area prioritas training.', placement: 'right' },
        { target: '.tour-deep-table', content: 'Performance Grid: Identifikasi departemen yang "High Risk" (Completion rendah, Budget tinggi).', placement: 'top' }
    ];

    useEffect(() => {
        if(isOpen) {
            const seen = localStorage.getItem('seen_analytics_tour');
            if(!seen) setRunAnalyticsTour(true);
        } else {
    ...
    const handleTourFinish = () => {
        setRunAnalyticsTour(false);
        localStorage.setItem('seen_analytics_tour', 'true');
    }
    ...
    return (
        <div className="fixed inset-0 z-[70] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-fadeIn">
            <Joyride 
                steps={analyticsSteps} run={runAnalyticsTour} continuous showSkipButton 
                callback={(data) => { if(data.status === STATUS.FINISHED || data.status === STATUS.SKIPPED) handleTourFinish() }}
                styles={{ options: { zIndex: 10000, primaryColor: KARSA_RED } }}
            />
            <div className="bg-white rounded-3xl w-full max-w-5xl h-[90vh] shadow-2xl flex flex-col animate-pop overflow-hidden relative">
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="tour-deep-radar bg-white p-4 rounded-xl border border-slate-100 shadow-sm">
                    ...
                    <div>
                        ...
                        <div className="tour-deep-table overflow-x-auto rounded-xl border border-slate-200">
                ...
                {/* Help Trigger */}
                <button onClick={() => setRunAnalyticsTour(true)} className="absolute bottom-5 left-1/2 -translate-x-1/2 text-xs text-slate-400 hover:text-slate-600 flex items-center gap-1"><HelpCircle size={12}/> Explain this chart</button>
            </div>
        </div>
    );
};
      ...
      {selectedEmp && (
        <div className={`fixed inset-0 lg:static lg:w-1/3 bg-white lg:rounded-2xl shadow-lg border-l lg:border border-slate-200 flex flex-col z-50 animate-slideInRight lg:animate-slideIn`}>
          ...
          <div className="pt-12 px-6 pb-6 flex-1 overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6"><div className="tour-emp-detail-stats bg-slate-50 p-3 rounded-xl border border-slate-100 text-center"><p className="text-xs text-slate-500 uppercase font-bold">Assigned</p><p className="text-xl font-bold text-slate-800">{selectedEmp.courses_assigned}</p></div><div className="bg-slate-50 p-3 rounded-xl border border-slate-100 text-center"><p className="text-xs text-slate-500 uppercase font-bold">Completed</p><p className="text-xl font-bold text-[#D12027]">{selectedEmp.courses_completed}</p></div></div>
...
const AnalyticsDashboard = ({ onOpenDeptDetail }) => (
    <div className="animate-fadeIn space-y-6 pb-20">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="tour-analytics-top bg-white p-6 rounded-2xl shadow-sm border border-slate-200 order-2 lg:order-1">
...
const DashboardOverview = ({ analyticsTrigger }) => (
  <div className="space-y-6 animate-fadeIn pb-20">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="tour-activity-chart lg:col-span-2 bg-white p-5 md:p-6 rounded-2xl shadow-sm border border-slate-200"><h3 className="font-bold text-lg text-slate-800 mb-6">Learning Activity Trends</h3><div className="h-64 md:h-72"><ResponsiveContainer width="100%" height="100%"><AreaChart data={[{ name: 'W1', hours: 400 }, { name: 'W2', hours: 300 }, { name: 'W3', hours: 500 }, { name: 'W4', hours: 450 }]}><defs><linearGradient id="colorHours" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#D12027" stopOpacity={0.1}/><stop offset="95%" stopColor="#D12027" stopOpacity={0}/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" /><XAxis dataKey="name" axisLine={false} tickLine={false} /><YAxis axisLine={false} tickLine={false} width={30}/><Tooltip contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}/><Area type="monotone" dataKey="hours" stroke="#D12027" fillOpacity={1} fill="url(#colorHours)" strokeWidth={3} /></AreaChart></ResponsiveContainer></div></div>
      <div className="tour-dept-progress bg-white p-5 md:p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col justify-between"><div><h3 className="font-bold text-lg text-slate-800 mb-6">Completion by Dept</h3><div className="space-y-6">{[{ name: 'Frontliner', val: 85 }, { name: 'Kitchen', val: 72 }, { name: 'Operational', val: 95 }, { name: 'Warehouse', val: 60 }].map((dept) => (<div key={dept.name}><div className="flex justify-between text-sm mb-2"><span className="font-medium text-slate-700">{dept.name}</span><span className="font-bold text-slate-800">{dept.val}%</span></div><ProgressBar value={dept.val} colorClass={dept.val > 80 ? "bg-green-500" : dept.val > 60 ? "bg-yellow-500" : "bg-red-500"} /></div>))}</div></div><button onClick={analyticsTrigger} className="tour-dashboard-analytics-btn w-full mt-8 py-3 text-sm text-[#D12027] font-bold border border-[#D12027] rounded-lg hover:bg-red-50 transition-colors flex items-center justify-center gap-2"><PieIcon size={16}/> View Detailed Analytics</button></div>
    </div>
  </div>
);

// --- 7. MAIN APP SHELL WITH DETAILED TUTORIAL LOGIC ---
...
const AdminApp = () => {
  ...
  const getSteps = (view) => {
      const sidebarStep = { target: '.tour-sidebar', content: 'Main Navigation: Gunakan sidebar ini untuk mengakses modul HR yang berbeda (Employee, Curriculum, dll).', placement: 'right' };
      ...
      switch(view) {
          case 'dashboard':
              return [
                  ...
                  { target: '.tour-activity-chart', content: 'Activity Trends: Grafik ini menunjukkan total jam belajar seluruh karyawan per minggu.', placement: 'top' },
                  { target: '.tour-dept-progress', content: 'Dept Completion: Perbandingan kepatuhan training antar divisi.', placement: 'left' },
                  { target: '.tour-dashboard-analytics-btn', content: 'Deep Dive: Klik tombol ini untuk melihat analisa Skill Gap yang lebih mendalam.', placement: 'top' }
              ];
          case 'employees':
              return [
                  { target: '.tour-emp-search', content: 'Smart Search: Cari karyawan berdasarkan Nama, Role, atau Departemen.', placement: 'bottom' },
                  { target: '.tour-emp-add', content: 'Onboarding: Tambahkan data karyawan baru secara manual di sini.', placement: 'left' },
                  { target: '.tour-emp-table', content: 'Employee List: Klik baris mana saja untuk melihat Detail Panel di sebelah kanan.', placement: 'top' },
                  // Note: Steps inside the detail panel are triggered contextually or by user interaction in a real app, 
                  // but here we focus on the list view structure.
              ];
          case 'curriculum':
              return [
                  { target: '.tour-curr-create', content: 'Course Builder: Buat modul baru (Video, PDF, atau SCORM). Mendukung sistem "Draft" sebelum publikasi.', placement: 'left' },
                  { target: '.tour-curr-assign', content: 'Bulk Assign: Kirim materi wajib ke seluruh Departemen dalam satu klik.', placement: 'bottom' },
                  { target: '.tour-curr-grid', content: 'Course Library: Kartu ini menunjukkan status (Draft/Active) dan jumlah staff yang sudah di-assign.', placement: 'top' }
              ];
          case 'requests':
              return [
                  { target: '.tour-req-table', content: 'Approval Workflow: Daftar pengajuan training eksternal dari karyawan.', placement: 'top' },
                  { target: '.tour-req-detail', content: 'Audit Trail: Klik "Detail" untuk melihat surat persetujuan atasan dan estimasi biaya sebelum menyetujui.', placement: 'left' }
              ];
          case 'analytics':
              return [
                  { target: '.tour-analytics-top', content: 'Leaderboard: Gamifikasi untuk memacu semangat belajar karyawan terbaik.', placement: 'right' },
                  { target: '.tour-analytics-budget', content: 'Budget Monitoring: Pastikan pengeluaran training tidak melebihi alokasi kuartal.', placement: 'left' },
                  { target: '.tour-analytics-deep', content: 'Fitur Premium: Klik di sini untuk membuka Modal Analisa Skill Gap (Radar Chart).', placement: 'top' }
              ];
          default: return [];
      }
  };
  ...
  useEffect(() => {
      // Auto-start tour only if not seen before for this view
      const seenKey = `hasSeen_${activeView}_v2`; 
  ...
  const handleJoyrideCallback = (data) => {
      ...
      if ([STATUS.FINISHED, STATUS.SKIPPED].includes(status)) {
          setRunTour(false);
          localStorage.setItem(`hasSeen_${activeView}_v2`, 'true');
      }
  };
      ...
      <div className="flex-1 flex flex-col h-screen relative w-full">
        <header className="h-16 lg:h-20 bg-white/80 backdrop-blur border-b border-slate-200 flex items-center justify-between px-4 lg:px-8 z-20 sticky top-0">
          ...
          <div className="flex items-center gap-4 lg:gap-6">
            <button onClick={startTourManually} className="text-sm font-bold text-[#D12027] bg-red-50 px-3 py-1.5 rounded-lg border border-red-100 hover:bg-red-100 transition-colors flex items-center gap-2"><HelpCircle size={16}/> Tutorial Mode</button>
